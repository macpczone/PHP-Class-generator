...
class StatusTypeDB {
	...
	public static function lookup($where = '1', $params = array()) {
		$sql = "SELECT * FROM status_type WHERE $where";
	...
	public function write() {
		...
		if ($this->get_id() == null) {
			...
			$sql = "INSERT INTO status_type(id, type) VALUES (null, ?)";
		} else {
			$sql = "UPDATE status_type SET id = ?, type = ? WHERE id = ?";
			$params[] = $this->id;
		}
	...
	public function delete() {
		$sql = "DELETE FROM status_type WHERE id = ?";
		try{
			$dbh = DB::getPdo(); 
			$stmt = $dbh->prepare($sql);
